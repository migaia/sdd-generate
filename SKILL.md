...
| `preflight.ts run --sdd <SDD> [--repository <root>] [--affected]` | Run the leaf's `preflight` items in disposable copies and write the report `validate` requires; this is testing, authorized for such leaves |
| `render.ts --sdd <SDD> [--check]` | Rewrite `sdd-generated` regions (exports, preserve, gates) from the contract |

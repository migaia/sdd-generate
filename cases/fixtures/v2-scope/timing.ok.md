...
## Acceptance
- A1 Timing 200 sequential `use` calls on hosts of 500 and 4000 plugins, the mean time per call grows by at most 3x (`packages/feature-a/scaling.test.ts`, run serially in its own `test:timing` script, outside the parallel default gate).

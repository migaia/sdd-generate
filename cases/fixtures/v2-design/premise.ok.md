...
## Steps
- S1 Create `packages/feature-a/index.ts` exporting `featureA()`; `use` binary-searches the member slot in the lane, then splices (the lane is kept slot-sorted by `packages/feature-a/index.ts:1`).

...
## Requirements
- R1 Every `use`/`unUse`/`dryRun` call stays within the dependency-computation bound of one node.
  - BC1 old behaviour: `mode` is `old`; new behaviour: `mode` is `new` (A1).
...
```json
{
  ...
  "preflight_report": "ok.md.preflight.json",
  "preflight": [

...
- Repository gate (root): `pnpm run fmt:check` → `pnpm -r --filter ./packages/feature-a run build` → `pnpm run test`; changes a gate writes outside packages/feature-a are reverted and reported separately.

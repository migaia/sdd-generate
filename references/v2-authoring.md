...
- **Preflight before handoff.** When the SDD has a BC, a scaling gate or a consumed export, execute what already runs against the current repository, within the user's authority (untested items are reported as unverified): the existing tests an inventory names, each gate command (watching for writes outside `writes`), and a scripted probe of every acceptance whose behaviour already exists, including whether its fixture's preconditions hold under the producer's verified semantics. Report every finding in one batch; delivery should not be the first run of the design.
- **Amendment closure.** After any amendment, run `validate` again and re-read every clause and inventory entry that cites a changed ID (`SDD_V2_AMENDMENT_CITATION_RECHECK` lists the inventory ones) before the SDD returns to the host.

...
- **Preflight before handoff.** A leaf with a BC, a scaling gate or a consumed export declares `preflight` items ([contract](v2-contract.md#compact-index)) and is not valid until `bun <create-sdd-root>/scripts/preflight.ts run --sdd <SDD>` has run them against the current code and every item passed (`SDD_V2_PREFLIGHT_REQUIRED`, `_STALE`, `_FAILED`). Items cover: each BC's candidate change as a patch that passes; each preserve acceptance run as is; each scaling gate clean (`expect: "pass"`) and with a perturbation patch it must catch (`expect: "fail"`); each acceptance that relies on a consumed export; and every repository gate command (`"gate": true`), whose writes outside `writes` fail it. Each item runs in a disposable copy with installed dependencies linked; nothing touches the repository.
- **Amendment closure.** The report is bound to the document digest, so any edit makes it stale; `validate` then lists the changed clauses and the clauses that cite them. Run `preflight.ts run --affected` (re-runs the items covering them plus every gate), re-validate, and re-review those clauses when a review is recorded.
